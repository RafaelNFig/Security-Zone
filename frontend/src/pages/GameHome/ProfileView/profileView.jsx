...
const ProfileView = () => {
  ...
  const { profile, loading, error, refetch, updateLocalProfile, isAuthenticated } =
    useProfile();

  // ✅ resolve playerId de forma robusta (evita "ID não disponível")
  const resolvedPlayerId = useMemo(() => {
    const fromProfile = profile?.PL_ID ?? profile?.id ?? profile?.playerId ?? null;
    if (fromProfile != null) return fromProfile;

    const cached = authUtils.getPlayerData?.();
    return cached?.PL_ID ?? cached?.id ?? cached?.playerId ?? null;
  }, [profile]);

  // ✅ Guarda de rota: se não está autenticado, vai pro login (sem loop)
  useEffect(() => {
    if (loading) return;
  ...
  }, [loading, error, isAuthenticated, profile, resolvedPlayerId, activeTab]);
  ...
  const handleUpdateProfile = useCallback(
    async (formData) => {
      try {
        // ✅ atualiza localmente (hook decide se também faz request)
  ...
  // Se não autenticado, o effect já redireciona; aqui só evita flash
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#070A10]">
        <div className="text-slate-300 text-sm">Redirecionando…</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#070A10]">
        <div className="text-center">
          ...
          <button
            onClick={handleBack}
            className="mt-4 px-6 py-2 bg-white/10 text-white rounded-xl"
          >
            Voltar
          </button>
        </div>
          ...
          <div className="w-full px-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <div className="space-y-2">
                <Stat label="Nível" value={profile.PL_LEVEL ?? 1} />
                <Stat
                  label="Moedas"
                  value={`${profile.PL_COINS ?? 0} 🪙`}
                  accent="yellow"
                />
                <Stat
                  label="Gemas"
                  value={`${profile.PL_GEMS ?? 0} 💎`}
                  accent="blue"
                />
                <Stat
                  label="Vida"
                  value={`${profile.PL_LIFE ?? 100}%`}
                  accent="green"
                />
                {profile.PL_EXPERIENCE !== undefined && (
                  <Stat label="EXP" value={profile.PL_EXPERIENCE} accent="purple" />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleRefresh}
                disabled={isRefreshing}
                className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 disabled:opacity-60"
                title="Atualizar"
              >
                <RefreshCw
                  size={18}
              ...
              {/* ✅ X volta para GameHome corretamente */}
              <button
                onClick={handleBack}
