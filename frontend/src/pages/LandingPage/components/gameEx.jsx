...
function pickByName(cards, name) {
  return cards.find((c) => String(c?.name || "").toLowerCase() === name.toLowerCase());
}

export default function GameEx({ onOpenAuth = () => {}, cards = [] }) {
  const floatingCards = useMemo(() => {
    const list = Array.isArray(cards) ? cards : [];

    const vpn = pickByName(list, "VPN Ativada");
    const fw = pickByName(list, "Firewall Básico");
    const up = pickByName(list, "Atualização de Software");

    const picked = [vpn, fw, up].filter(Boolean);

    // fallback: pega 3 primeiras com imagem
    if (picked.length < 3) {
      const extra = list.filter((c) => c?.img).slice(0, 3 - picked.length);
      picked.push(...extra);
    }
